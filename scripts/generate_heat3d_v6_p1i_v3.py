#!/usr/bin/env python3
"""Versioned V6-P1i v3 generator adapter.

The frozen v0/v1/v2 generator and core are imported read-only. This adapter
changes only the v3 power law, split binding, and label-independent support
coverage policy.
"""
...
DEFAULT_CONFIG = (
    ROOT / "configs/heat3d_v6_p1i/v6_p1i_pilot128_v3.yaml"
)
...
def case_from_sobol_v3(
    ...
    if (
        ...
        != "continuous_severity_top_h_power_law_with_independent_jitter"
    ):
        raise core.ContinuousPhysicsError("v3 power distribution mismatch")
    base_power = base._lerp(power_spec["base_range_W"], severity)
    top_h_factor = (
    ...
    if multiplier_spec["distribution"] != "log_uniform":
        raise core.ContinuousPhysicsError("v3 independent multiplier mismatch")
    ...
    if not low <= power <= high:
        raise core.ContinuousPhysicsError(
            f"v3 global power rule produced {power} W outside {low, high}"
        )
