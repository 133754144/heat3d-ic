...
def main():
 ...
 p.add_argument('--population-mode',choices=['frozen_valid32','remaining_valid96'],required=True);p.add_argument('--population-preflight',type=Path);p.add_argument('--runs-root',type=Path,required=True);p.add_argument('--seed0-run-dir',type=Path,required=True);a=p.parse_args();q=json.loads(a.protocol.read_text());count=32 if a.population_mode=='frozen_valid32' else 96;seeds=[0] if count==32 else q['confirmation']['seeds'];orders=q['performance']['randomized_order_seeds'] if count==32 else [None];run_dirs={0:a.seed0_run_dir,1:a.runs_root/'V6_07_V5best_P1i_seed1_reliable_B24',2:a.runs_root/'V6_08_V5best_P1i_seed2_reliable_B24'};state={'schema_version':'heat3d_v6_p1i_post_freeze_neural_matrix_v1','status':'running','population_mode':a.population_mode,'cells':[],'role_contract':q['role_contract']};a.output_root.mkdir(parents=True,exist_ok=True);write(a.output_root/'execution_state.json',state)
 for seed in seeds:
