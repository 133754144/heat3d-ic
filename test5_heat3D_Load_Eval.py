...
# 2. Or run:
#      RIGNO_SHOW_XLA_WARNINGS=1 python3 test5_heat3D_Load_Eval.py
#
...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="Load a trained 3D heat RIGNO checkpoint and evaluate it on the test split.",
  )
  parser.add_argument(
    "--checkpoint",
    type=Path,
    default=(repo_dir / "output" / "heat3d_rigno" / "heat3d_rigno_best.pkl"),
  )
...
def main() -> None:
  ...
  model = RIGNO(**payload["model_config"])
  ...
  # ---------------------------------------------------------------------------
  # Author-style Relative L1 error from example.py
  # ---------------------------------------------------------------------------
