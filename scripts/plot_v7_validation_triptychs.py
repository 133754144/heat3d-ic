...
def draw(root, names, basename, title):
    ...
    if dense:
        ...
        display_mask=(Delaunay(points).find_simplex(query)<0).reshape(256,256)
        fallback=float(display_mask.mean())
        mode=f'Native 1,024 nodes; same-layer thin-plate RBF ({len(source)} nodes) to 256 x 256; display only; {fallback:.1%} outside support hull MASKED'
    ...
    if display_mask is not None:
        truth=np.ma.array(truth,mask=display_mask)
        predictions=[np.ma.array(p,mask=display_mask) for p in predictions]
        errors=[p-truth for p in predictions]
    lo=min(float(truth.min()),*(float(p.min()) for p in predictions)); hi=max(float(truth.max()),*(float(p.max()) for p in predictions))
    temperature_norm=Normalize(lo,hi)
    temperature_cmap=plt.get_cmap('turbo').copy(); temperature_cmap.set_bad('#d9dde2')
    error_cmap=plt.get_cmap('RdBu_r').copy(); error_cmap.set_bad('#d9dde2')
    reference_rgba=temperature_cmap(temperature_norm(truth))
    reference_checks=[]
    ...
    for i,(name,p,e,m) in enumerate(zip(names,predictions,errors,meta)):
        for j,v in enumerate([truth,p,e]):
            ax=fig.add_subplot(gs[i,j]); im=ax.imshow(v,origin='lower',extent=extent,aspect='equal',interpolation='bilinear',cmap=temperature_cmap if j<2 else error_cmap,norm=temperature_norm if j<2 else Normalize(-errmax,errmax))
            ...
            if j==0:
                actual_rgba=im.to_rgba(im.get_array())
                if not np.array_equal(actual_rgba,reference_rgba): raise ValueError('reference RGBA differs between models')
                reference_checks.append(hashlib.sha256(actual_rgba.tobytes()).hexdigest())
            ...
            for b in source_boxes:
                x0,x1,y0,y1=b['bbox_fraction_xy']; x0,x1=[extent[0]+v*(extent[1]-extent[0]) for v in (x0,x1)]; y0,y1=[extent[2]+v*(extent[3]-extent[2]) for v in (y0,y1)]
    ...
    foot='All rows: identical reference pixels and common reference/prediction temperature limits; each panel has its own colorbar.\nDashed boxes: actual heat-source footprints in the displayed layer. Shared symmetric error limits. Gray: unsupported spatial extrapolation (masked).\nFull checkpoint paths, hashes, sample provenance and commands: '+basename+'_manifest.json + README.md'
    if dense: foot=foot.replace('Dashed boxes: actual heat-source footprints in the displayed layer. ', '').replace(' Gray: unsupported spatial extrapolation (masked).', '')
    ...
    manifest=dict(title=title,sample=meta[0]['sample_id'],z=z,layer=layer,interpolation=mode,outside_hull_fraction=fallback,temperature_limits_by_row=[[lo,hi] for p in predictions],reference_rgba_sha256_by_row=reference_checks,reference_rgba_identical=len(set(reference_checks))==1,unsupported_pixels_masked=display_mask is not None,source_boxes=source_boxes,error_limits=[-errmax,errmax],rows=meta,plot_script_sha256=sha(__file__))
