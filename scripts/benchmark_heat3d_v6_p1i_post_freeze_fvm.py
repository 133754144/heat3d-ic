...
def main():
 p=argparse.ArgumentParser();p.add_argument('--protocol',type=Path,required=True);p.add_argument('--dataset-root',type=Path,required=True);p.add_argument('--manifest',type=Path,required=True);p.add_argument('--full-fields',type=Path,required=True);p.add_argument('--process-counts',default='1,2,4');p.add_argument('--output',type=Path,required=True);a=p.parse_args();q=json.loads(a.protocol.read_text());counts=[int(value) for value in a.process_counts.split(',')];orders=[np.random.default_rng(seed).permutation(32).tolist() for seed in q['performance']['randomized_order_seeds']];rows=[]
 for count in counts:
