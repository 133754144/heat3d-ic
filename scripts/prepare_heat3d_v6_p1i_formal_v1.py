#!/usr/bin/env python3
"""Prepare and freeze target-independent V6-P1i formal1024_v1 inputs."""
...
import generate_heat3d_v6_p1i_v15 as adapter
...
def prepare(
    ...
    pilot_audit_path = (
        CONFIG_DIR / "v6_p1i_pilot128_v15_distribution_audit.json"
    )
    pilot_closeout_path = CONFIG_DIR / "v6_p1i_pilot128_v15_closeout.json"
    pilot_audit = json.loads(pilot_audit_path.read_text(encoding="utf-8"))
    pilot_closeout = json.loads(
        pilot_closeout_path.read_text(encoding="utf-8")
    )
    if (
        pilot_audit.get("status") != "passed"
        or not all(pilot_audit.get("checks", {}).values())
        or pilot_closeout.get("status")
        != "generated_complete_qualification_passed"
        or not pilot_closeout["decision"]["pilot_acceptance_passed"]
    ):
        raise RuntimeError(
            "formal1024_v1 inputs remain forbidden until pilot128_v15 "
    ...
    report = {
        "schema_version": "heat3d_v6_p1i_formal1024_v1_preflight_v1",
        ...
        "guardrails": {
            ...
            "pilot128_v15_all_gates_passed": True,
        },
    }
...
def freeze(
    ...
    artifacts = [
        ...
        CONFIG_DIR / "v6_p1i_v14_population_power_calibration.json",
        CONFIG_DIR / "v6_p1i_v15_population_power_calibration.json",
        CONFIG_DIR / "v6_p1i_pilot128_v14_distribution_audit.json",
        CONFIG_DIR / "v6_p1i_pilot128_v14_closeout.json",
        CONFIG_DIR / "v6_p1i_pilot128_v15_distribution_audit.json",
        CONFIG_DIR / "v6_p1i_pilot128_v15_closeout.json",
        paths["split"],
        paths["inputs"],
        paths["preflight"],
    ...
    payload = {
        "schema_version": "heat3d_v6_p1i_formal1024_v1_freeze_v1",
