...
def main() -> int:
    ...
    parser.add_argument(
        "--compact-integration",
        action="store_true",
        help="Validate immutable provenance bindings without requiring excluded source payloads.",
    )
    ...
    for source in provenance_sources:
        if args.compact_integration:
            require(
                len(source["sha256"]) == 64
                and all(char in "0123456789abcdef" for char in source["sha256"]),
                f"source SHA binding: {source['path']}",
            )
            require(
                not (ROOT / source["path"]).exists(),
                f"excluded source payload entered compact integration: {source['path']}",
            )
        else:
            require(
                sha256(ROOT / source["path"]) == source["sha256"],
                f"source SHA: {source['path']}",
            )
