...
def boundary_features(
    ...
    side = np.isclose(x, 0) | np.isclose(x, 1) | np.isclose(y, 0) | np.isclose(y, 1)
...
def htc_case(
    ...
    metadata = {
        ...
        "target_contract": "deltaT_K=25*u_if_same_reference_scaling_is_used",
        "no_solver_run": True,
    }
...
def volume_v1_case(power_path: Path, target_path: Path | None, index: int) -> tuple[dict[str, np.ndarray], dict]:
    ...
    z = coords[:, 2]
    k_scalar = np.full(len(coords), 0.1, dtype=np.float32)
    ...
    if target_path is not None:
        u = select_array(target_path, (101, 101, 56), index)
        arrays["deltaT_K"] = (25.0 * u).reshape(-1, 1)
    metadata = {
        ...
        "target_scaling": "temperature_K=293.15+25*u; deltaT_K=25*u",
