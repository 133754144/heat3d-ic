...
#      RIGNO_SHOW_XLA_WARNINGS=1 python3 test4_heat3D_Train_Save.py
#
...
if os.environ.get("RIGNO_SHOW_XLA_WARNINGS", "").lower() in {"1", "true", "yes", "on"}:
...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="Train a steady 3D heat RIGNO model and save the best checkpoint.",
  )
  parser.add_argument("--datadir", type=Path, default=(repo_dir / "dataset_3d_heat"))
  parser.add_argument("--output-dir", type=Path, default=(repo_dir / "output" / "heat3d_rigno"))
  parser.add_argument("--checkpoint-name", type=str, default="heat3d_rigno_best.pkl")
...
def main() -> None:
  ...
  dataset = Heat3DDataset(str(args.datadir))
