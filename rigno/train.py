...
def train(
  ...
  # Set constants
  num_samples_trn = dataset.nums['train'] # 训练样本数量
  num_times = dataset.shape[1] # 每条轨迹的时间长度 T
  num_pnodes = dataset.shape[2] # 空间点数 N
  num_vars = dataset.shape[3] # 变量数
  assert num_samples_trn % FLAGS.batch_size == 0 # 确保训练集能被 batch_size 整除
  num_batches = num_samples_trn // FLAGS.batch_size 
  assert FLAGS.batch_size % NUM_DEVICES == 0 # 确保 batch 能均匀分到每张 GPU
  ...
  if dataset.time_dependent:
    num_init_times = num_times - 1 # 一个轨迹有 T 个快照，能做“起点”的时间索引是0..T-2（最后一帧不能再往后预测）
    assert num_init_times > 0
    assert tau_max < num_times # 最大跨度不能超过轨迹长度
  ...
  # Define the steppers - 决定模型训练时的目标是什么
  if FLAGS.stepper == 'der': # 学时间导数（更像学 dT/dt）
    stepper = TimeDerivativeStepper(operator=model)
  elif FLAGS.stepper == 'res': # 学残差（T(t+τ)-T(t)）
    stepper = ResidualStepper(operator=model)
  elif FLAGS.stepper == 'out': # 直接输出 T(t+τ) 或稳态 T
  ...
  def _train_one_batch(
    ...
    def _update_state_per_subbatch(
      ...
      def _get_loss_and_grads(
        ...
        def _compute_loss(
          ...
          _loss_inputs = stepper.get_loss_inputs(
            ...
            u_tgt=u_tgt,
            inputs=inputs,
            graphs=graph_builder.build_graphs(batch.g), # 从 batch 的图元数据构造真正可用的图结构
            key=subkey,
          )

          return loss_fn(*_loss_inputs) # 返回均方误差

        # Split tau for unrolling - 为 tau 的每个元素随机生成一个分割比例 frac
        if unroll: # 当 unroll=True 时，模型会先“走一段随机长度”（tau_inf）的推理路径，再基于剩余部分计算损失，从而迫使模型学习更鲁棒的多步预测。
          # Choose a random split fraction
          key, subkey = jax.random.split(key) # 把当前的随机种子 key 分裂成两个独立的子种子
          frac = jax.random.uniform(key=subkey, shape=tau.shape) # 生成一个形状与 tau 完全相同的数组，元素值均匀分布在 [0, 1) 区间
          ...
          if isinstance(stepper, TimeDerivativeStepper):
            ...
            key, subkey = jax.random.split(key)
            P = .5
            coin = jax.random.uniform(key=subkey, shape=tau.shape) > P # 50%概率分割
            frac = frac * coin
          ...
          tau_inf = tau_cutoff_approximation + frac * tau_splittable_part # tau_inf ∈ [tau_cutoff_approximation, tau - tau_cutoff_inference]，随机分布
          # Avoid inference with tau=0 if necessary
          if isinstance(stepper, TimeDerivativeStepper): # 如果 tau_inf 恰好等于最小保留值 tau_cutoff_approximation（即没有真正分割），就强制设为 0
      ...
      # Apply gradients
      state = state.apply_gradients(grads=grads) # 用优化器更新参数（AdamW）
    ...
    if dataset.time_dependent:
      ...
      u_inp_batch = jax.vmap( # 向量化映射，对从每个起点 lt 取输入快照
          lambda lt: jax.lax.dynamic_slice_in_dim( # 动态切片
            operand=batch.u,
      ...
      u_tgt_batch = jax.vmap( # 对每个起点 lt，取未来 tau_max 个目标快照
          lambda lt: jax.lax.dynamic_slice_in_dim(
      ...
      # Remove the invalid pairs - 删除无效配对
  ...
  def train_one_epoch( # 遍历所有的batch
    key: flax.typing.PRNGKey,
    state: TrainState,
  ...
  def _evaluate_direct_prediction(
    ...
    """
    # 小步长：模型可能学得不好直接大步预测，所以用 unroll 多小步模拟；大步长：直接单步预测
    if tau_ratio < 1:
      assert is_multiple(1., tau_ratio) # 检查tau_ratio是否可以被1整除（确保可以整除，比如 0.5 → 2 步模拟 1）
      _tau_ratio = 1
    else:
      step = lambda *args, **kwargs: stepper.unroll(*args, **kwargs, num_steps=int(1 / tau_ratio)) 
      assert isinstance(tau_ratio, int) # 检查是否为整数
      step = stepper.apply
    ...
    _u_pair = (batch.u[:, _tau_ratio:], u_prd[:, :-_tau_ratio]) # 把真值和预测对齐
  ...
  def _evaluate_final_prediction(
    ...
    if dataset.time_dependent:
      ...
      idx_fn = IDX_FN // FLAGS.time_downsample_factor # 选 final 的时间索引
      u_tgt = batch.u[:, (idx_fn):(idx_fn+1)] # 取 final 时刻的真值温度场
      inputs = Inputs(
        u=batch.u[:, :1],
        c=(batch.c[:, :1] if (batch.c is not None) else None),
        x_inp=batch.x,
  ...
  def evaluate(
    state: TrainState,
    batches: Iterable[Batch],
    direct: bool = True, # 是否评估“一步预测”
    rollout: bool = False, # 是否评估“整条轨迹自回归 rollout”
    final: bool = True, # 是否只评估“最终时刻 final”
  ) -> EvalMetrics:
    ...
    # Turn off unrelevent evaluations - 如果数据不是时间相关，关掉 direct/rollout
...
def main(argv):
  ...
  if not FLAGS.datetime:
    FLAGS.datetime = datetime.now().strftime('%Y%m%d-%H%M%S') # 时间格式化
  ...
  # Split the epochs
  if schedule_tau_max:
    epochs_warmup = int(.2 * FLAGS.epochs) # warmup 用总 epochs 的 20%
