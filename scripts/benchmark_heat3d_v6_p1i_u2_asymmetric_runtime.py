...
import heat3d_v6_supplemental_runtime as supplemental  # noqa: E402
...
# Supplemental orchestration may install a read-only hook.  It runs outside
# the frozen production timing span and cannot alter model execution.
SUPPLEMENTAL_RESULT_HOOK = None
...
def parse() -> argparse.Namespace:
    ...
    parser.add_argument("--supplemental-input-plan", type=Path)
    parser.add_argument("--supplemental-static-reuse", action="store_true")
...
def main() -> int:
    ...
    preflight = None if supplemental_plan is not None else json.loads(preflight_path.read_text())
    if preflight is not None and preflight["sample_ids"] != [x.sample_id for x in anchors]: raise RuntimeError("population order drift")
    ...
    physics_rows = (supplemental_plan["physics"] if supplemental_plan is not None
                    else {row["sample_id"]: row for row in preflight["samples"]})
    ...
    if args.timing_only and supplemental_plan is None:
    ...
    for anchor in ([] if (args.timing_only or supplemental_plan is not None) else anchors):
        with np.load(physics_rows[anchor.sample_id]["physics_cache_file"], allow_pickle=False) as physics:
    ...
    def prepare_one(
        ...
        static_geometry: Any = None, static_sink: Any = None,
        prepare_only: bool = False,
    ) -> dict[str, Any]:
        nonlocal packing_prediction_audit_done, exactness_audit_done
        if static_geometry is None and not prepare_only:
            static_geometry = supplemental_static.get(anchor.sample_id)
        ...
        if static_geometry is None:
            ...
            selected, selected_cv = static_geometry["selected"], static_geometry["selected_cv"]
        support_s = time.perf_counter()-phase
        phase=time.perf_counter()
        query_support = {"selected_indices": selected, "operator_control_volume": selected_cv,
                         "k_xyz": full_k[selected], "q_W_m3": full_q[selected], "layer_id": layer[selected]}
        query_example = highn._query_example(anchor, query_support, coords)
        builder = ((static_geometry or {}).get("builder")
                   or Heat3DGraphBuilder(**graph_config))
        ...
        with jax.default_device(cpu):
            ...
            if static_geometry is None:
                ...
                native=static_geometry["native"]
            anchor_graph_build_s=time.perf_counter()-phase
            phase=time.perf_counter()
            if static_geometry is None: block(native)
            anchor_graph_sync_s=time.perf_counter()-phase
            ...
            if static_geometry is not None:
                asymmetric, audit = static_geometry["asymmetric"], static_geometry["audit"]
                query_graph_build_s=time.perf_counter()-phase
            elif args.asymmetric_mode == "u_v2":
                asymmetric,audit=u1.prior_u1._u_v2_asymmetric_metadata(
                    builder,native,anchor_coords,query_coords,
                    numerical_tolerance=float(protocol["u_v2"]["normalized_numerical_tolerance"]),
        ...
        phase=time.perf_counter()
        if static_geometry is not None:
            mapping = static_geometry["mapping"]
        else:
            mapping=None if direct else build_reconstruction_map(
        ...
        if static_sink is not None:
            static_sink.update({
                "selected": selected, "selected_cv": selected_cv,
                "builder": builder, "native": native, "asymmetric": asymmetric,
                "audit": audit, "mapping": mapping,
            })
        if prepare_only:
            return {"prepared_static_geometry": True}
        ...
        if SUPPLEMENTAL_RESULT_HOOK is not None:
            SUPPLEMENTAL_RESULT_HOOK(
                anchor=anchor,
                payload=(inputs_in, inputs_out, graphs, local, kwargs, map_indices, map_weights),
                prediction=full_np,
                stages=dict(stage_payload), elapsed_seconds=float(production_elapsed),
            )
        ...
        return {"sample_id":anchor.sample_id,"inputs_in":inputs_in if retain_device else None,"inputs_out":inputs_out if retain_device else None,"graphs":graphs if retain_device else None,"local":local if retain_device else None,
                ...
                "supplemental_static_artifacts":({
                    "support_sha256":tree_sha(selected),
                    "cv_sha256":tree_sha(selected_cv),
                    "graph_payload_sha256":tree_sha((graphs,local)),
                    "map_sha256":tree_sha((map_indices,map_weights)),
                } if supplemental_plan is not None else None),
                "stages":stage_payload,"timing_audit":timing_payload,
                "asymmetric_graph_audit":audit,
                "shape":shape_payload}

    supplemental_static: dict[str, dict[str, Any]] = {}
    if supplemental_plan is not None and args.supplemental_static_reuse:
        for anchor in anchors:
            sink: dict[str, Any] = {}
            prepare_one(
                anchor, qualification_audit=False, record_completion=False,
                static_sink=sink, prepare_only=True,
            )
            supplemental_static[anchor.sample_id] = sink
    ...
    if args.publication_v1_1 and not args.concurrent_only:
        fresh_values = [float(row["stages"]["matched_continuous_e2e"]) for row in prepared]
        lifecycle_metrics = serial_metrics(
            cold_seconds=fresh_values[0], fresh_q1=dist(fresh_values),
            cache_hot=dist(cache_hot), resident=dist(valid_resident),
        )
    elif args.publication_v1_1 and args.concurrent_only:
        if true_concurrent is None:
            raise RuntimeError("Q2 lifecycle missing true concurrent result")
        lifecycle_metrics = q2_metrics(
            submit_to_result=true_concurrent["submit_to_result"],
            inter_completion=true_concurrent["inter_completion"],
            throughput_samples_per_second=true_concurrent["samples_per_second"],
            b16_to_b32_marginal_seconds=true_concurrent["actual_B16_to_B32_marginal_seconds"],
        )
    result={"schema_version":"heat3d_v6_p1i_u2_asymmetric_runtime_cell_v1","status":"passed" if args.sample_count in (32,96) else "passed_smoke",
        "route": "U_v2_direct240825" if direct else "U_v2_16384_reconstruction",
        "service_mode": "Q2" if args.concurrent_only else "serial",
        ...
        "runtime":{"fresh_sample":timing if not args.concurrent_only else None,
                   "same_input_replay":dist(replay) if replay else None,
                   "repeat_case_cache_hot":dist(cache_hot) if cache_hot else None,
                   "repeat_case_cache_hot_rows":cache_hot_rows,
                   "resident_core":dist(valid_resident) if valid_resident else None},"batch":batch_rows,
        "lifecycle_metrics": lifecycle_metrics,
        "timing_pool_classification":{"cold_service_first_case":"first_serial_row" if not args.concurrent_only else None,"fresh_distinct_case":"serial_rows" if not args.concurrent_only else None,"repeat_case_cache_hot":"separate_post_fresh_declared_case_cache_pool" if args.publication_v1_1 and not args.concurrent_only else None,"resident_core":"separate_valid_prepared_device_payload_pool" if args.publication_v1_1 and not args.concurrent_only else None,"pools_mixed":False},
        "streaming":({"submit_to_result":dist([r["streaming"]["submit_to_result_seconds"] for r in prepared]),"inter_completion":dist([r["streaming"]["inter_completion_seconds"] for r in prepared]),"wall_seconds":completion_offsets[-1],"samples_per_second":len(prepared)/completion_offsets[-1],"order_seed":args.order_seed} if not args.concurrent_only else None),
