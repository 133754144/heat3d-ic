#!/usr/bin/env python3
"""Versioned V6-P1i v13 generator adapter.

The frozen v0/v1/v2 generator and core are imported read-only. This adapter
retains the split/support contract and uses one frozen population-level,
monotonic piecewise-linear severity curve. It never reads targets when
constructing a case.
"""
...
DEFAULT_CONFIG = (
    ROOT / "configs/heat3d_v6_p1i/v6_p1i_pilot128_v13.yaml"
)
...
def case_from_sobol_v3(
    ...
    if (
        ...
        != "monotonic_piecewise_severity_top_h_power_law_with_independent_jitter"
    ):
        raise core.ContinuousPhysicsError("v13 power distribution mismatch")
    curve = power_spec["severity_curve"]
    latent_knots = np.asarray(curve["latent_knots"], dtype=np.float64)
    power_knots = np.asarray(curve["base_power_knots_W"], dtype=np.float64)
    if (
        latent_knots.ndim != 1
        or power_knots.shape != latent_knots.shape
        or len(latent_knots) < 2
        or latent_knots[0] != 0.0
        or latent_knots[-1] != 1.0
        or np.any(np.diff(latent_knots) <= 0.0)
        or np.any(np.diff(power_knots) <= 0.0)
    ):
        raise core.ContinuousPhysicsError(
            "v13 severity curve must be strictly monotonic on [0, 1]"
        )
    base_power = float(np.interp(severity, latent_knots, power_knots))
    top_h_factor = (
    ...
    if multiplier_spec["distribution"] != "log_uniform":
        raise core.ContinuousPhysicsError("v13 independent multiplier mismatch")
    ...
    if not low <= power <= high:
        raise core.ContinuousPhysicsError(
            f"v13 global power rule produced {power} W outside {low, high}"
        )
