...
- cross-host publication archive receipt is tracked separately; the Mac mirror is
  ...
  long-term release；
- `codex/g2-curated-integration@230980986...` 已原样推送，但 direct-parent/source-alignment
  audit 为 FAIL_CLOSED；没有 merge 或 fast-forward `research/v7`，也没有创建 closeout tag。

本轮 visualization、跨主机 archive 与 allowlist 内容审计已通过；curated integration
因 parent/source 对齐硬门槛未通过而保持阻塞，`research/v7` ref 保持不变。
