...
def main() -> int:
    ...
    native_reload_max_abs = safe_tree_difference(prediction, reloaded_prediction)
    native_reload_allclose = native_reload_max_abs <= 1.0e-5
    ...
    for row_index, example in enumerate(examples["valid"]):
        ...
        reload_max_abs = float(np.max(np.abs(full_high - full_high_reload)))
        u_rows.append({"sample_id": example.sample_id, "slice_output_count": int(len(high)), "full_output_count": int(full_high.size), "finite": bool(np.all(np.isfinite(full_high)) and np.all(np.isfinite(truth))), "checkpoint_reload_allclose_atol_rtol_1e5": bool(np.allclose(full_high, full_high_reload, atol=1e-5, rtol=1e-5)), "checkpoint_reload_max_abs": reload_max_abs, "reconstruction_algorithm": audit["algorithm"], "U_full_reconstruction_wall_seconds": reconstruction_seconds})
    status = "PASS_1_EPOCH_DUAL_OUTPUT_NONFORMAL" if checkpoint["passed"] and native_reload_allclose and native_finite and np.isfinite(float(valid_loss)) and all(row["finite"] and row["checkpoint_reload_allclose_atol_rtol_1e5"] for row in u_rows) else "FAIL"
    receipt = {"schema_version": "heat3d_v7_g2_p6_heat3d_v1_three_output_preflight_v3", "status": status, "scope": "one_optimizer_step_schema_frozen_normalization_native_and_reconstruction_query_checkpoint_only", "formal_accuracy_claim": False, "device_backend": jax.default_backend(), "cuda_required": args.require_cuda, "train_functions": 6, "valid_functions": 2, "epochs": 1, "optimizer_steps": 1, "normalization_payload_sha256": normalization_sha, "support": {"input_count": 1024, "selector": "physics_layout_aware_source_interface_boundary_volume", "old_8x8x16_projection_used": False}, "native_output": {"count": 1024, "finite": native_finite, "checkpoint_reload_allclose_atol_rtol_1e5": native_reload_allclose, "checkpoint_reload_max_abs": native_reload_max_abs}, "u_strategy_outputs": {"conditioning_count_remains": 1024, "source_slice": {"count": 10201, "shape": [101,101], "z": 0.15}, "full_primary": {"count": 571256, "shape": [101,101,56], "same_domain_as_official_full_output": True}, "latency_includes_reconstruction": True, "rows": u_rows}, "validation_loss_finite": bool(np.isfinite(float(valid_loss))), "checkpoint": {**checkpoint, "sha256": sha256(checkpoint_path)}, "temporary_artifact_root": str(args.output_dir), "formal_or_long_training_started": False, "p1i_test_or_sealed_access": False}
