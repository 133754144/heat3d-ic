...
def main()->int:
    ...
    result={"schema_version":"heat3d_v6_p1i_p5_adaptive_preprocessing_closeout_v1","status":"passed" if hard else "failed","protocol_sha256":hashlib.sha256(a.protocol.read_bytes()).hexdigest(),"hard_gate_passed":hard,"population":protocol["population"],"timing_semantics":{"graph_shape_warmup_before_each_reference_and_candidate":True,"graph_total":"continuous build_metadata plus full metadata block_until_ready","total":"support ordering + CV redistribution + synchronized graph total + reconstruction map"},"summary":summary,"samples":rows,"decision":{"optimized_adaptive_preprocessing":"GO","standalone_packing_optimization":"NO_GO_candidate_packing_regressed_about_3ms_but_synchronized_graph_total_improved","gpu_tiled":"NO_GO","next_stage":"GO_batch_or_offline_parallel_support_ordering; NO_GO_more_graph_policy_or_GPU_tiled_search"},"role_contract":protocol["role_contract"]};a.output_json.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
    ...
    lines += ["## Timing semantics and decision","","Reference and candidate graph paths were warmed separately for every exact shape. `graph_total` is one continuous `build_metadata` span followed by a full metadata `block_until_ready`; the total is ordering + CV + synchronized graph + reconstruction map. Qualification, hashes, inference and serialization are excluded.","","All 64 route/sample cells passed support-order, CV, canonical-graph and reconstruction-map exact gates. Candidate packing alone regressed by about 3 ms and is therefore **NO-GO as a standalone optimization**. Exact reverse reuse is retained because synchronized graph total still improves 1.49x for B8192 and 1.98x for E32768. Keep the CPU sparse path; do not resume GPU-tiled or graph-policy search. The next justified engineering step is batch/offline parallel support-order preparation; inference batching remains a separate later study."]
