#!/usr/bin/env python3
"""Deterministic V6-RandomBlock mesh, field, support, and FVM utilities.
...
SCHEMA_VERSION = "heat3d_v6_randomblock_dataset_v1"
AMBIENT_K = 300.0
ACTIVE_LAYERS = ("silicon_die_lower", "silicon_die_upper")
TEMPERATURE_BIN_EDGES_K = (30.0, 60.0, 90.0, 120.0, 150.0)
K_PALETTE_W_MK = (20.0, 50.0, 120.0, 135.0, 140.0, 150.0, 385.0, 400.0, 600.0)


class RandomBlockError(RuntimeError):
    """Raised when a frozen RandomBlock contract is violated."""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != SCHEMA_VERSION:
        raise RandomBlockError(f"{path}: unexpected RandomBlock schema")
    return payload
...
def _axis_widths(axis: np.ndarray) -> np.ndarray:
    ...
    if axis.ndim != 1 or axis.size < 2 or np.any(np.diff(axis) <= 0.0):
        raise RandomBlockError("mesh axis must be finite, 1-D and increasing")
...
def _build_z_axis(
    ...
    for index, layer in enumerate(layers):
        ...
        if thickness <= 0.0 or intervals < 1:
            raise RandomBlockError("invalid layer thickness/z intervals")
...
def build_mesh(physics: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for layer in layers:
        if "k_xyz_W_mK" in layer:
            value = list(map(float, layer["k_xyz_W_mK"]))
        else:
            scalar = float(layer["k_W_mK"])
            value = [scalar, scalar, scalar]
        if len(value) != 3 or min(value) <= 0.0:
            raise RandomBlockError(f"non-positive layer conductivity: {layer}")
    ...
    if not set(ACTIVE_LAYERS) <= set(layer_index):
        raise RandomBlockError("frozen active layers are missing")
    return {
        "x": x,
        "y": y,
        "z": z,
...
def block_mask(
    ...
    if layer not in ACTIVE_LAYERS:
        raise RandomBlockError(f"block outside active layers: {layer}")
    bbox = list(map(float, block["bbox_fraction_xy"]))
    if len(bbox) != 4:
        raise RandomBlockError("bbox_fraction_xy must contain x0,x1,y0,y1")
    x0, x1, y0, y1 = bbox
    if not (0.0 <= x0 < x1 <= 1.0 and 0.0 <= y0 < y1 <= 1.0):
        raise RandomBlockError(f"invalid bbox: {bbox}")
...
def validate_layout(
    ...
    if not (2 <= len(k_blocks) <= 8 and 3 <= len(q_blocks) <= 10):
        raise RandomBlockError(f"{group['group_id']}: block-count contract")
    if _same_layer_overlap(k_blocks) or _same_layer_overlap(q_blocks):
        raise RandomBlockError(f"{group['group_id']}: illegal same-family overlap")
...
def build_case_fields(
    ...
    if len(k_values) != len(group["k_blocks"]):
        raise RandomBlockError("k block/value length mismatch")
    if len(q_fractions) != len(group["q_blocks"]):
        raise RandomBlockError("q block/fraction length mismatch")
    if any(value not in K_PALETTE_W_MK for value in k_values):
        raise RandomBlockError("uncited k value in case")
    if min(q_fractions) <= 0.0 or not math.isclose(
        sum(q_fractions), 1.0, rel_tol=0.0, abs_tol=1.0e-12
    ):
        raise RandomBlockError("q fractions must be positive and sum to one")
    ...
    if not np.all(np.isfinite(k_diag)) or float(np.min(k_diag)) <= 0.0:
        raise RandomBlockError("k field is not finite positive definite")
    ...
    for block_index, (mask, fraction) in enumerate(
        ...
        if not (14.5 - 1.0e-9 <= surface_density <= 1000.0 + 1.0e-9):
            raise RandomBlockError(
                f"{case['sample_id']}: surface density {surface_density}"
            )
        if density > 6.666666666666667e10 * (1.0 + 1.0e-12):
            raise RandomBlockError(f"{case['sample_id']}: q above literature envelope")
        rows.append(
            {
                "family": "q",
                "block_index": block_index,
...
def select_group_support(
    ...
    if len(selected) != 1024 or Counter(strata) != {
        ...
        raise RandomBlockError("support quota invariant failed")
...
def boundary_flags(coords: np.ndarray, mesh: Mapping[str, Any]) -> np.ndarray:
    ...
    if not np.all(np.sum(result, axis=1) == 1.0):
        raise RandomBlockError("boundary flags are not one-hot")
    return result
...
def solve_case(
    ...
    *,
    ...
    if min(top_h, bottom_h) <= 0.0:
        raise RandomBlockError("dual-Robin h must be positive")
    ...
    if info != 0:
        raise RandomBlockError(f"CG failed: info={info}")
    ...
    if not np.all(np.isfinite(temperature)):
        raise RandomBlockError("non-finite temperature")
    return temperature, {
...
def temperature_bin(delta_t_peak_K: float) -> int | None:
    value = float(delta_t_peak_K)
    edges = TEMPERATURE_BIN_EDGES_K
    if value < edges[0] or value > edges[-1]:
        return None
    if value == edges[-1]:
        return len(edges) - 2
    return int(np.searchsorted(np.asarray(edges), value, side="right") - 1)
...
def case_metrics(
    ...
    *,
    ...
    return {
        ...
        "realized_temperature_bin": temperature_bin(peak),
