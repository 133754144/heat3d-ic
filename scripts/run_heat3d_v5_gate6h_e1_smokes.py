#!/usr/bin/env python3
"""Run Gate 6H e1-only compile/reload/memory smokes from registered e600 configs."""
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--summary-json",
        type=Path,
        default=ROOT / "output/heat3d_v5_gate6h_smoke/e1_smoke_summary.json",
    )
...
def _resolved(config_id: str) -> dict[str, Any]:
    ...
    resolved["run"].update({
        ...
        "memory_audit_jsonl": f"output/heat3d_v5_gate6h_smoke_memory/{config_id}.jsonl",
        "final_probe_output_dir": f"output/heat3d_v5_gate6h_smoke_final_probe/{config_id}",
        "post_training_diagnostics_output_dir": f"output/heat3d_v5_gate6h_smoke_diagnostics/{config_id}",
    })
    resolved["export"].update({
        "output_dir": f"output/heat3d_v5_gate6h_smoke_runs/{config_id}",
...
def _run(config_id: str) -> dict[str, Any]:
    ...
    if loss_summary_path.is_file():
        prior = json.loads(loss_summary_path.read_text(encoding="utf-8"))
        complete = bool(
            prior.get("checkpoint_prediction_reload_audit", {}).get("status") == "passed"
            and prior.get("post_training_diagnostics_result", {}).get("status") == "completed"
        )
    if not complete:
    ...
    loss = json.loads(loss_summary_path.read_text(encoding="utf-8"))
    ...
    reload_audit = loss["checkpoint_prediction_reload_audit"]
    assert reload_audit["summary_persisted_before_replay"] is True
    assert run_config["checkpoint_prediction_reload_audit"]["summary_persisted_before_replay"] is True
    diagnostics = loss["post_training_diagnostics_result"]
    assert diagnostics["enabled"] is True
    assert diagnostics["status"] == "completed"
    assert diagnostics["scope"] == "valid_iid_only"
    assert diagnostics["forbidden_roles_accessed"] == []
    ...
    return {
        ...
        "post_training_diagnostics_status": diagnostics["status"],
        "post_training_diagnostics_scope": diagnostics["scope"],
        "grad_finite": True,
...
def main() -> int:
    ...
    for config_id in selected:
        ...
        payload = {
            "schema_version": "heat3d_v5_gate6h_e1_smoke_v1",
