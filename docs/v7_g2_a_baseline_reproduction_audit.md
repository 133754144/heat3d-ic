...
| Therm-FM | Poseidon/scOT 的网格热场 foundation-model adaptation | 官方 tiny demo 的 loader 与 3-epoch training PASS，但 evaluation 发生环境 Abort trap 6；其网格张量和 checkpoint 语义不能直接当作 P1i point-cloud adapter |
| DeepOHeat | 3D-IC 配置编码与 DeepONet/physics-aware path | 官方 `src.modules.DeepONet` 原生 CPU forward PASS；数据/checkpoint convention 尚未与 P1i 对齐 |
...
- DeepOHeat 的官方模型 forward 已通过，但 P1i adapter 仍 deferred；Therm-FM 只完成 tiny demo 的部分复线，DeepOHeat-v2 仍是 paper-only。它们均未被伪称为正式 baseline PASS。
