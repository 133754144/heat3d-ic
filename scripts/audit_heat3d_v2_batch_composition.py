...
def _category_counts(sample_ids: tuple[str, ...], example_by_id: dict[str, Any]) -> dict[str, dict[str, int]]:
    ...
    for field in CATEGORY_FIELDS:
        counter = Counter(
            _meta_value(example_by_id[sample_id].meta, field)
...
def _batch_record(index: int, group: dict[str, Any], example_by_id: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "category_counts": _category_counts(sample_ids, example_by_id),
...
def _write_md(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "| batch | samples | total_edges | stack_template | source_category | k_region_mode | bc_category | target_delta_mean | target_delta_std |",
        "| --- | ---: | ---: | --- | --- | --- | --- | ---: | ---: |",
    ]
    for batch in payload["batches"]:
        counts = batch["category_counts"]
        lines.append(
            "| {idx} | {samples} | {edges} | {stack} | {source} | {k_region} | {bc} | {mean:.6e} | {std:.6e} |".format(
                ...
                mean=batch["target_delta"]["mean"],
                std=batch["target_delta"]["std"],
            )
        )
...
def main() -> int:
    ...
    sample_root = _sample_root(Path(dataset_config["subset_path"]))
    split_ids, split_source, primary_validation_split, stress_validation_split = _resolve_training_splits(
        sample_root,
        Path(split_map_path) if split_map_path else None,
    )
    ...
    ordered_groups = _epoch_train_groups(groups, epoch=args.epoch, seed=batch_order_seed, shuffle=shuffle)
    batches = [
        _batch_record(index, group, example_by_id)
    ...
    payload = {
        ...
        "train_sample_count": int(len(train_examples)),
