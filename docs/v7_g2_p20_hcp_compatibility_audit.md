...
状态：`NEEDS_AMENDMENT`；资产决策：`BLOCKED_BY_UPSTREAM_ASSETS`；正式范围暂为 `REFERENCE_ONLY`。本轮只审计官方代码/论文接口，不训练、不下载 checkpoint/raw outputs、不访问 test/sealed。
...
因此，P1i 的 irregular coordinates、heterogeneous `k(x)`、q/source 布局和双 Robin BC 没有被证明可以被原实现原生接收。大幅重写 HCP branch/input/interface 后再称“original HCP baseline”不被接受。本 track 选择 `REFERENCE_ONLY / BLOCKED_BY_UPSTREAM_ASSETS`，后续优先复现 HCP native single-HTC benchmark；只有在取得官方资产并验证 native evaluation 后，才评估同域 Heat3D adaptation。
