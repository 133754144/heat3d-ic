"""Classes and metadata for reading the datasets.""" # 数据集读取，元数据定义，预处理
...
class Metadata:
  ...
  periodic: bool # 空间是否为周期性边界
  group_u: str
  group_c: str
  group_x: str
  type: Literal['poseidon', 'rigno'] # 规则网格 or 点云
  fix_x: bool # 坐标是否固定
  domain_x: tuple[Sequence[int], Sequence[int]] # 空间范围
  domain_t: tuple[int, int] # 时间范围
  ...
  names: dict[str, Sequence[str]] # 变量名字
...
DATASET_METADATA = {
  ...
  'unstructured/Poisson-Gauss': Metadata(
    periodic=False,
    group_u='u',
    group_c='c',
    group_x='x',
...
class Batch(NamedTuple):
  ...
  u: Array # 解
  c: Union[None, Array] # 系数场
  x: Array # 空间坐标
  t: Union[None, Array] # 时间坐标
  g: Union[None, Sequence[RegionInteractionGraphSet]] # 图结构

  @property
  def shape(self) -> tuple:
    return self.u.shape

  def unravel(self) -> tuple: # 方便一次性读取核心4个数组
    return (self.u, self.c, self.x, self.t)

  def __len__(self) -> int: # 返回batch size
    return self.shape[0]
...
class Dataset:
  ...
  def __init__(self,
    datadir: str,
    datapath: str,
    include_passive_variables: bool = False, # 默认false：只调用训练需要的变量
    concatenate_coeffs: bool = False,
    time_cutoff_idx: int = None, # 截断时间长度
    time_downsample_factor: int = 1, # 时间下采样：每隔k帧取一帧
    space_downsample_factor: float = 1., # 空间随机下采样
    n_train: int = 128,
    n_valid: int = 16,
    n_test: int = 0,
    preload: bool = False, # 是否一次性把数据读取到内存
    key: PRNGKey = None,
  ):
    """
    Args:
    ...
    self.reader = h5py.File(Path(datadir) / f'{datapath}.nc', 'r') # 直接打开HDF5文件
    ...
    self.shape = self.sample.shape # 先取一个样本确定统一的shape
  ...
  def compute_stats(self, residual_steps: int = 0) -> None:
    ...
    assert residual_steps < self.shape[1] # self.shape[1]是时间长度

    # Get all trajectories
    batch = self.train(np.arange(self.nums['train']))
    u, c, _, t = batch.unravel()

    # Compute statistics of solutions and coefficients - 对batch、时间、空间都求均值，保留变量维度 V
    ...
    if self.time_dependent:
      ...
      for s in range(1, residual_steps+1):
        res = _get_res(s, u) # u(t+s)-u(t)
        tau = _get_res(s, t) # t(t+s)-t(t)
      ...
      self.stats['res']['std'] = np.std(residuals, axis=(0, 1, 2), keepdims=True) # 计算标准差
  ...
  def build_graphs(self, builder: RegionInteractionGraphBuilder, rmesh_correction_dsf: int = 1, key: PRNGKey = None) -> None:
    ...
    metadata = []
    num_p2r_edges = 0 # 物理点→区域点（p2r）
    num_r2r_edges = 0 # 区域点→区域点多尺度边（r2r）
    num_r2p_edges = 0 # 区域点→物理点（r2p）
    ...
    for mode in ['train', 'valid', 'test']:
      ...
      for x in batch.x[:, 0]: # 假设坐标不随时间变化，所以只取第 0 个时间步的坐标
    ...
    # Pad the edge sets using dummy nodes - 把“每个样本边数不同”的图元数据 pad 成统一 shape
    ...
    self.rigs = tree.tree_map(lambda *v: jnp.concatenate(v), *metadata) # 把所有图元数据拼成一个大数组树
