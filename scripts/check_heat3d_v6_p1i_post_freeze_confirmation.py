...
def main():
 ...
 if a.performance:
  r=json.loads(a.performance.read_text());req(r['status']=='passed_with_u_direct_fail_closed','performance');req(r['output_domain_nodes']==240825,'performance contract');req(all(x['randomized_order_count']>=3 and x['exact_model_semantics_across_orders'] for x in r['performance'].values()),'exact gates');req(r['role_contract']==q['role_contract'],'performance roles');
  if a.performance_csv:req(len(list(csv.DictReader(a.performance_csv.open())))>=4,'performance CSV');checked.append('performance')
 if a.confirmation:
  r=json.loads(a.confirmation.read_text());req(r['status']=='passed_with_u_direct_fail_closed','confirmation');req(r['valid32_subset_formal_valid128'] and r['valid96_count']==96,'population');req(set(r['confirmation_valid96_three_seed_mean_std'])=={'E16384_reconstruction','E240825_direct'},'routes');req(set(r['paired_bootstrap'])=={'seed0','seed1','seed2'},'seeds');req(r['u_direct_valid96']['status']=='fail_closed_not_evaluated' and not r['u_direct_valid96']['accuracy_or_CI_available'],'U fail closed');req(r['role_contract']==q['role_contract'],'confirmation roles');
  if a.confirmation_csv:req(len(list(csv.DictReader(a.confirmation_csv.open())))==6,'confirmation CSV');checked.append('confirmation')
