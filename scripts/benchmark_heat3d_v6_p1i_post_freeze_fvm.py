...
def main():
 ...
 for process_count in counts:
  ...
  for repeat,order in enumerate(orders):
   ...
   prefix16=float(sum(completion[:16])) if len(completion)>=16 else None
   prefix32=float(sum(completion[:32])) if len(completion)>=32 else None
   repeats.append({'repeat':repeat,'order_seed':seeds[repeat],'order':order,'fresh_wall_seconds':fresh_wall,'fresh_samples_per_second':sample_count/fresh_wall,'fresh_single_case':stats([x['continuous_compute_seconds'] for x in fresh]),'resident_core_wall_seconds':resident_wall,'resident_core_solve_only':stats([x['solve_seconds'] for x in resident]),'streaming_wall_seconds':stream_wall,'streaming_samples_per_second':sample_count/stream_wall,'stream_submit_to_result':stats(submit_to),'stream_inter_completion':stats(completion),'stream_completion_intervals_seconds':completion,'true_B16_wall_seconds':prefix16,'true_B32_wall_seconds':prefix32,'true_B16_to_B32_marginal_seconds':None if prefix16 is None or prefix32 is None else (prefix32-prefix16)/16.0})
  pool.shutdown();rows.append({'process_count':process_count,'startup_and_prepare_all_seconds':startup,'untimed_warmup':True,'repeats':repeats,'fresh_population_wall':stats([x['fresh_wall_seconds'] for x in repeats]),'resident_population_wall':stats([x['resident_core_wall_seconds'] for x in repeats]),'streaming_population_wall':stats([x['streaming_wall_seconds'] for x in repeats]),'fresh_throughput':stats([x['fresh_samples_per_second'] for x in repeats]),'streaming_throughput':stats([x['streaming_samples_per_second'] for x in repeats])})
 saturation=max(rows,key=lambda x:x['streaming_throughput']['median_seconds']);result={'schema_version':'heat3d_v6_p1i_post_freeze_fvm_v2','status':'passed','output_nodes':240825,'population_mode':a.population_mode,'sample_count':sample_count,'sample_ids':sample_ids,'queue_depth':a.queue_depth,'rows':rows,'tested_process_counts':counts,'process_count_cap_reason':'persistent prepared-system workers are bounded by host RAM; cap is resource-feasibility, not performance selection','saturation_process_count':saturation['process_count'],'randomized_order_count':len(orders),'role_contract':q['role_contract']};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n');print(json.dumps({'status':'passed','saturation':saturation['process_count'],'streaming_samples_s':saturation['streaming_throughput']['median_seconds']}));return 0
