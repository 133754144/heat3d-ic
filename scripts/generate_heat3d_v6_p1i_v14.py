#!/usr/bin/env python3
"""Versioned V6-P1i v14 generator adapter.
...
DEFAULT_CONFIG = (
    ROOT / "configs/heat3d_v6_p1i/v6_p1i_pilot128_v14.yaml"
)
...
def case_from_sobol_v3(
    ...
    if (
        ...
        raise core.ContinuousPhysicsError("v14 power distribution mismatch")
    ...
    if (
        ...
        raise core.ContinuousPhysicsError(
            "v14 severity curve must be strictly monotonic on [0, 1]"
        )
    ...
    if multiplier_spec["distribution"] != "log_uniform":
        raise core.ContinuousPhysicsError("v14 independent multiplier mismatch")
    ...
    if not low <= power <= high:
        raise core.ContinuousPhysicsError(
            f"v14 global power rule produced {power} W outside {low, high}"
        )
