...
# Supplemental orchestration may install a read-only hook.  It is invoked only
# after the frozen production timer has stopped and never changes the payload.
...
def main() -> int:
    global SUPPLEMENTAL_RESULT_HOOK
    args = parse()
    ...
    if supplemental_plan is not None:
        def _capture_supplemental(**row: Any) -> None:
            sample_id = row["anchor"].sample_id
            if sample_id in supplemental_capture:
                return
            prediction = np.asarray(row["prediction"], dtype=np.float32)
            supplemental_capture[sample_id] = {
                "prepared_payload_sha256": tree_sha(tuple(
                    row["payload"][key] for key in (
                        "anchor", "query", "weights", "map_indices", "map_weights"))),
                "prediction": prediction,
                "prediction_sha256": hashlib.sha256(
                    np.ascontiguousarray(prediction).tobytes()).hexdigest(),
                "elapsed_seconds": row["elapsed_seconds"],
                "stages": row["stages"],
                "static_artifacts": {
                    "support_sha256": tree_sha(row["payload"]["_supplemental_static_refs"]["selected"]),
                    "cv_sha256": tree_sha(row["payload"]["_supplemental_static_refs"]["selected_cv"]),
                    "native_graph": metadata_hashes(row["payload"]["_supplemental_static_refs"]["anchor_metadata"]),
                    "query_graph": metadata_hashes(row["payload"]["_supplemental_static_refs"]["query_metadata"]),
                    "map_sha256": tree_sha((row["payload"]["map_indices"], row["payload"]["map_weights"])),
                },
            }
        SUPPLEMENTAL_RESULT_HOOK = _capture_supplemental
    ...
    if len(anchors) != (4 if supplemental_plan is not None else 32):
    ...
    def prepare_host(
        ...
        include_untimed_hash_audit: bool = False, static_geometry: Any = None,
        static_sink: Any = None,
        ...
        if static_geometry is not None:
            selected = static_geometry["selected"]
            selected_cv = static_geometry["selected_cv"]
        elif direct:
        ...
        anchor_builder = (static_geometry or {}).get("anchor_builder") or Heat3DGraphBuilder(**anchor_config)
        query_builder = (static_geometry or {}).get("query_builder") or Heat3DGraphBuilder(**query_config)
        ...
        with jax.default_device(cpu):
            ...
            if static_geometry is None:
                ...
                anchor_metadata = static_geometry["anchor_metadata"]
            ...
            if static_geometry is None:
                ...
                query_metadata = static_geometry["query_metadata"]
        ...
        phase = time.perf_counter()
        if static_geometry is not None:
            map_indices = static_geometry["map_indices"]
            map_weights = static_geometry["map_weights"]
        elif direct:
        ...
        if supplemental_plan is not None:
            payload["_supplemental_static_refs"] = {
                "selected": selected, "selected_cv": selected_cv,
                "anchor_metadata": anchor_metadata, "query_metadata": query_metadata,
            }
        ...
        if static_sink is not None:
            static_sink.update({
                "selected": selected, "selected_cv": selected_cv,
                "anchor_builder": anchor_builder, "query_builder": query_builder,
                "anchor_metadata": anchor_metadata, "query_metadata": query_metadata,
                "map_indices": map_indices, "map_weights": map_weights,
            })
    ...
    if supplemental_plan is not None and args.supplemental_static_reuse:
        for anchor in anchors:
            sink: dict[str, Any] = {}
            prepare_host(anchor, static_sink=sink)
    ...
    def service_one(anchor: Any, *, cache_hot: bool = False) -> dict[str, Any]:
        ...
        if cache_hot:
            ...
            payload, stages = prepare_host(
                anchor, static_geometry=supplemental_static.get(anchor.sample_id))
        ...
        if SUPPLEMENTAL_RESULT_HOOK is not None:
            SUPPLEMENTAL_RESULT_HOOK(
                anchor=anchor, payload=payload, prediction=np.asarray(prediction),
                stages=dict(stages), elapsed_seconds=float(elapsed),
            )
        return {
    ...
    orders = {seed: np.random.default_rng(seed).permutation(len(anchors))[:args.sample_count].tolist()
    ...
    if supplemental_plan is not None:
        ...
        ordered = [anchor.sample_id for anchor in anchors]
        np.savez_compressed(
            prediction_path,
            sample_ids=np.asarray(ordered),
            full_deltaT_K=np.stack([supplemental_capture[sample_id]["prediction"] for sample_id in ordered]),
        )
        result["supplemental_capture"] = {
            "prediction_path": str(prediction_path),
            "prediction_sha256": hashlib.sha256(prediction_path.read_bytes()).hexdigest(),
            "rows": [{key: value for key, value in supplemental_capture[sample_id].items()
                      if key != "prediction"} for sample_id in ordered],
        }
        SUPPLEMENTAL_RESULT_HOOK = None
