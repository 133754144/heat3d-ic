...
def main():
 ...
 if a.performance:
  r=json.loads(a.performance.read_text());req(r['status']=='completed_frozen','performance');req(r['output_nodes']==240825 and r['randomized_order_count']>=3,'performance contract');req(r['exact_gates']['all_passed'],'exact gates');req(r['role_contract']==q['role_contract'],'performance roles');
  if a.performance_csv:req(len(list(csv.DictReader(a.performance_csv.open())))>=4,'performance CSV');checked.append('performance')
 if a.confirmation:
  r=json.loads(a.confirmation.read_text());req(r['status']=='completed_frozen','confirmation');req(r['population']['valid32_subset_formal_valid128'] and r['population']['valid96_count']==96,'population');req(set(r['seeds'])=={0,1,2},'seeds');req(r['role_contract']==q['role_contract'],'confirmation roles');
