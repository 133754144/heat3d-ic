...
def main():
 ...
 result={"schema_version":"heat3d_v6_p1i_u_v2_geometry_audit_v1","status":"passed","sample_count":96,"temperature_labels_read":False,"sample_ids":[x.sample_id for x in anchors],"rows":rows,"summary":{"outside_sample_count":sum(r["outside_node_count"]>0 for r in rows),"outside_node_count_total":sum(r["outside_node_count"] for r in rows),"outside_node_ratio_mean":float(np.mean([r["outside_node_ratio"] for r in rows])),"maximum_normalized_overshoot":max(r["maximum_normalized_overshoot"] for r in rows),"raw_uncovered_count_total":sum(r["raw_uncovered_count"] for r in rows),"repaired_uncovered_count_total":sum(r["repaired_uncovered_count"] for r in rows),"native_exact_all":all(all(r["native_exact"].values()) for r in rows)},"role_contract":q["role_contract"]}
